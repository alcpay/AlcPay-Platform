...
import { FormModalComponent } from '../../components/form-modal.ts/form-modal.ts.component';
import { Product } from '../../../models/product.model';

@Component({
  standalone: true,
  imports: [CommonModule, RouterModule, FormModalComponent],
  template: `
    <div class="min-h-screen bg-gray-100 p-6">
      ...
      <section class="mt-6 p-6 flex flex-col gap-4 bg-white rounded-lg shadow-lg">
        <span class="h4 section-heading">AlcPay Components</span>
          ...
          <!-- Drawer end -->
          <button type="button" class="btn btn-primary py-2 px-4" aria-haspopup="dialog" aria-expanded="false" aria-controls="overlay-end-example" data-overlay="#overlay-end-example">Toggle end</button>
          ...
          <!-- Drawer bottom -->
          <button type="button" class="btn btn-primary px-4 py-2" aria-haspopup="dialog" aria-expanded="false" aria-controls="overlay-bottom-example" data-overlay="#overlay-bottom-example">Toggle bottom</button>
    ...
    <dl class="divide-y divide-base-content/25">
            ...
            <li class="flex items-center justify-between py-4 ps-4 pe-5">
              <div class="flex w-0 flex-1 items-center">
                ...
                <div class="ms-4 flex min-w-0 flex-1 gap-2">
                  ...
                  <span class="flex-shrink-0 text-base-content/50">3.2mb</span>
                </div>
              </div>
            ...
            <li class="flex items-center justify-between py-4 ps-4 pe-5">
              <div class="flex w-0 flex-1 items-center">
                ...
                <div class="ms-4 flex min-w-0 flex-1 gap-2">
                  ...
                  <span class="flex-shrink-0 text-base-content/50">5.1mb</span>
                </div>
              </div>
              <div class="ms-4 flex-shrink-0">
                <a href="#" class="link link-primary">Download</a>
          ...
          <ng-container *ngFor="let heading of headings">
            <div class="flex flex-col items-center justify-center">
              <div class="block p-2 flex-grid items-center justify-center">
                <div class="{{ heading.class }} {{ heading.text }}">{{ heading.text }}</div>
              </div>
              <div class="w-full bg-gray-100 text-gray-600 p-2 mt-2 text-sm text-center text-nowrap overflow-x-clip">
                <code class="text-nowrap overflow-x-scroll text-xs">&lt;{{ heading.tag }} class="{{ heading.class }}"&gt;{{ heading.tag }}&lt;/{{ heading.tag }}&gt;</code>
              </div>
            </div>
          </ng-container>
        </div>
        ...
        <div class="grid grid-cols-4 gap-2 mt-4">
          <ng-container *ngFor="let button of buttons">
            <div class="flex flex-col items-center justify-center">
              ...
              <div class="w-full text-sm bg-gray-50 text-gray-800 py-2 px-4 mt-4 text-center">
                <code>&lt;button class="{{ button.class }}"&gt;{{ button.label }}&lt;/button&gt;</code>
              </div>
            </div>
          </ng-container>
        </div>
      ...
      <section class="mt-6 p-6 bg-white rounded-lg shadow-lg">
        ...
        <ng-container *ngFor="let grid of grids">
          <h3 class="text-md font-semibold text-gray-600 mt-4">{{ grid.title }}</h3>
          ...
          <ng-container *ngFor="let card of cards">
            <div class="bg-white p-6 rounded-lg shadow-lg">
              ...
              <ng-container *ngIf="card.button">
                <button class="mt-4 bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 transition">{{ card.button }}</button>
              </ng-container>
              <ng-container *ngIf="card.image">
                <img [alt]="card.imageAlt" class="w-full h-32 object-cover rounded-md mt-2" [src]="card.image" />
              </ng-container>
...
export default class TailwindPage {
  ...
  selectConfig = {
    ...
    toggleTag: "<button type=\"button\" aria-expanded=\"false\"><span class=\"me-2\" data-icon></span><span class=\"text-base-content\" data-title></span></button>", // HTML template for the toggle button
    ...
    optionTemplate: "<div><div class=\"flex items-center\"> <div class=\"me-2\" data-icon></div> <div class=\"font-semibold text-base-content\" data-title></div> </div> <div class=\"mt-1.5 text-sm text-base-content/80\" data-description></div> </div>", // HTML template for each option
    extraMarkup: "<span class=\"icon-[tabler--caret-up-down] flex-shrink-0 size-4 text-base-content absolute top-1/2 end-3 -translate-y-1/2 \"></span>" // Additional HTML markup for the select component
  };
   ...
   */
  icons: string[] = [
    'phone', 'email', 'person', 'settings', 'warning',
    'home', 'search', 'shopping_cart', 'star', 'check_circle',
    'close', 'delete', 'storefront', 'payments', 'shopping_bag',
    'account_circle', 'account_box', 'account_balance', 'account_balance_wallet',
  ];

  // Array of outlined icons for the icons section
  outlinedIcons: string[] = ['phone', 'email', 'lock', 'person', 'settings', 'info', 'warning'];
  ...
  cards = [
    ...
    { title: 'Card with Button', description: 'Cards can include buttons and actions.', button: 'Click Me' },
    { title: 'Card with Image', description: 'Cards can include images.', image: '/analog.svg', imageAlt: 'Analog Logo' },
    { title: 'Card with Image and Button', description: 'Cards can include images and buttons.', image: '/analog.svg', imageAlt: 'Analog Logo', button: 'Click Me' },
    { 
      title: 'Login Card', 
